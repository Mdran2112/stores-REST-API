...
def create_app(db_url: Optional[str] = None):
    ...
    def check_if_token_in_blocklist(jwt_heaer, jwt_payload):
        return jwt_payload["jti"] in BLOCKLIST

    @jwt.revoked_token_loader
    def revoked_token_callback(jwt_header, jwt_payload):
        """callback when check_if_token_in_blacklist returns True"""
        return (
            jsonify(
                {"description": "The token has been removed", "error": "tokenr_revoked"}
            )
        ), 401
    ...
    def add_claims_to_jwt(identity):
        # TODO Look in the database and see wether the user is an admin
        if identity == 1:
